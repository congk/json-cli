...
const getProterty = require('./edit/edit_get');
...
module.exports = (obj, path) => {
    ...
    rl.on('line', (line) => {
        // 查看是否是特殊命令
        const isCommand = rl.param.commands.some((command) => command.handle(line));
        if (isCommand) {
            return;
        }
        // 赋值操作
        if (/^[0-9a-zA-Z_\s]*={1,2}[^=].*$/.test(line)) {
            trace(`赋值操作 ===> ${line}`);
            rl.prompt();
            return;
        }
        ...
        if (/^[a-zA-Z-0-9_]+(.[a-zA-Z-0-9_]+)*$/.test(key)){
            getProterty(rl.param.target, key)
        } else {
            trace(`输入无法解析: ${line}`);
        }
        rl.prompt();
